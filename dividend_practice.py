...
class Stock(object):
    ...
    def _evaluate_roe(self):
        ...
        """
        balance_sheet_data = self._ticker.balance_sheet.to_dict().values()
        stockholder_equity = [v['Total Stockholder Equity']
                            for v in balance_sheet_data if 'Total Stockholder Equity' in v]
        financials_data = self._ticker.financials.to_dict().values()
        net_income = [v['Net Income']
                    for v in financials_data if 'Net Income' in v]
        
        if not stockholder_equity or not net_income:
